...
def VudioToText(file_path):
    with open("iFlytekConfig.json") as f:
        config = json.loads(f.read())
    appid = str(config["app_id"])
    secret = str(config["secret_key"])

    RequestApi(appid=appid, secret_key=secret, upload_file_path=file_path)
    return RequestApi.all_api_request()
# 注意：如果出现requests模块报错："NoneType" object has no attribute 'read', 请尝试将requests模块更新到2.20.0或以上版本(本demo测试版本为2.20.0)
# 输入讯飞开放平台的appid，secret_key和待转写的文件路径
if __name__ == '__main__':
    api = RequestApi(appid="d98275c9", secret_key="f9db6d5370583244bf5d61a843c451ad", upload_file_path=r"lfasr_涉政.wav")
    api.all_api_request()
